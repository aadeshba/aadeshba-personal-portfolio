...
private:
     ...
     *
     * In particular, you may want a function to find the larges company
     * that a student i is part of.
     */
    Company* largest(int i);

    // check to see if the baseCompany (student j) has an ancestor w/ the supplied address

    bool hasParent(Company* parentToTest, int baseCompany);
};
